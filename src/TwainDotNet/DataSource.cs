...
namespace TwainDotNet
{
	internal class DataSource : IDisposable
	{
		...
		/// </summary>
		static readonly ILog log = LogManager.GetLogger( typeof( DataSource ) );
		...
		public void NegotiateTransferCount( ScanSettings scanSettings )
		{
			try
			{
				...
				{
					if( Capability.GetCapability( Capabilities.XferCount, Message.Get, _applicationId, SourceId ) is BasicCapabilityResult basicCapabilityResult )
					{
		...
		private void NegotiateCapability( Capabilities capabilities, Fix32 fix32 )
		{
			...
			if( capabilityResult is ArrayCapabilityResult )
			{
		...
		*/
		...
		public bool Enable( ScanSettings settings )
		{
			...
			{
				ShowUI = ( short )( !uiControllable || settings.ShowTwainUI ? 1 : 0 ),
				ModalUI = 1,
		...
		public void Close()
		{
			if( SourceId.Id != 0 )
			{
				if( this.State >= 5 )
				{
					...
					{
						ConditionCode conditionCode = DataSourceManager.GetConditionCode( _applicationId, SourceId );
						log.Info( $"DisableDS {SourceId.ProductName} {result}: {conditionCode}." );
					}
				}
		...
		public void CloseSource()
		{
			if( SourceId.Id != 0 )
			{
				if( this.State >= 4 )
				{
					...
					{
						ConditionCode conditionCode = DataSourceManager.GetConditionCode( _applicationId, SourceId );
						log.Info( $"CloseSource {SourceId.ProductName} {result}: {conditionCode}." );
					}
				}
			}
		}
